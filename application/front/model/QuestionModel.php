...
class QuestionModel extends Model
{
    ...
    *   查找所有题目
    ...
    public function findAllQuestion(){
        $m_question = Db::name('ex_question');
        try{
            $result = $m_question->select();
            if(true == $result ){
    ...
    *       插入题库
    ...
    public function addQuestion($questionInfo){
        $m_question = Db::name('ex_question');
        try{
            $result = $m_question->insert($questionInfo);
            if(true == $result ){
    ...
    *          查找单个题库
     */
    public function findQuestion($questionInfo){

        $m_question = Db::name('ex_question');
        try{
            $result = $m_question->where($questionInfo)->find();
            if(true == $result ){
                return \common::errorArray(0, "查找成功", $result);
            }else{
                return \common::errorArray(1, "查找失败", $result);
    ...
    *       修改题目
     */
    function updateQuestion($condition,$questionInfo){
        $m_question = Db::name('ex_question');
        try{
            $result = $m_question->where($condition)->update($questionInfo);
            if(true == $result ){
                return \common::errorArray(0, "查找成功", $result);
            }else{
                return \common::errorArray(1, "查找失败", $result);
    ...
    *   删除单条题目
     */
    public function deleteQuestion($condtion){
        $m_question = Db::name('ex_question');
        try{
            $result = $m_question->where($condtion)->delete();
            if(true == $result ){
    ...
    *   分页展示题库
     */
    public function pagingQuestion($page,$conditionList,$sort = '',$orList = null){
        ...
        if(count($conditionList) > 0){
            ...
            $sql = "SELECT * FROM ex_question WHERE{$whereString} ";
            if($orString){
                $sql = "{$sql}AND {$orString} ";
            }
        }else{
            if($orString){
                $sql = "SELECT * FROM ex_question WHERE {$orString}";
            }else{
                $sql = "SELECT * FROM ex_question ";
            }
        }
        //排序
        if(null != $sort && '' != $sort){
        ...
        $sqlTotal =  "{$sql}{$sort}";
        try {
            $result['dataList'] =$m_question->query($sqlLimit);

            $result['sum'] = $m_question->query($sqlTotal);
            $sql = "SELECT count(*) as total_record_num  from ( {$sql} ) as count_table";
            $count = $m_question->query($sql);
