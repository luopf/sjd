$(function(){
    ...
    function userDetail(){
        var id = $(this).attr("data-id");
        window.location.href = host+'/index.php/admin/Company/CompanyDetail/cid/'+id;
    }
    /***
     * 删除公司
     * */
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:host+'/index.php/admin/Company/pagingCompany',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1">序号</th><th class="th3">工作单位名称</th><th class="th10">添加时间</th><th class="th11">操作</th></tr>';
                    var colspan = $(html).find("th").length;
                    for(var i = 0; i < userList.length;i++){
                        var obj = userList[i];
                        var number = (pageIndex - 1)*pageSize + i + 1;//序号

                        var name = obj.name?obj.name:"--"; // 公司名


                        var add_time = obj.add_time?obj.add_time:"--";// 添加时间


                        var id = obj.id;
                        html+='<tr>'
                        		+'<td>'+number+'</td>'
                        		+'<td><span class="limit-text" title="'+name+'">'+name+'</span></td>'
                        		+'<td>'+add_time+'</td>'
                        		+'<td>'
